...
НАУЧНОЕ ОБОСНОВАНИЕ:
    ...
    2. "Подход, применённый для одной задачи, не может ожидаемо работать
       с оптимальной точностью на других датасетах или модальностях."
    3. На ряде датасетов оригинальные (необработанные) данные давали лучший
       результат — это прямо мотивирует генерацию вариантов разной интенсивности
       для эмпирического выбора оптимального.

    Параметрический диапазон ×0.5–×2.0:
    Выбран эвристически как практически обоснованный диапазон умеренного
    изменения параметров. Конкретный оптимум определяется эмпирически —
    через обучение модели на каждом из созданных вариантов датасета.
